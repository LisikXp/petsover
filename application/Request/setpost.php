...
if ($breed != undefined && $location != undefined) {
	...
	if (isset($_POST['ajax_new_family'])) {
		...
		$avatar = $_POST['oldimgpet'];
		$profile = $users->add_another_dog_profile($username, $avatar, $breed, $bdate, $sex);

		$avatarfamily = $_POST['oldimfam'];
...
//echo $curr_password .  $password;
	...
	if (isset($_POST['ajax_post_photo_popup_main_url'])) {
		echo $wall->get_main_photo_popup();
	}
	...
	if (isset($_POST['search_main'])) {
		...
		$search->get_all_users($search_val, 0, 10);
	}

	if (isset($_POST['search_keyword'])) {
		$search = new Search;
		$search_val = $_POST['search_key_val'];
		$search->get_all_search_by_keyword($search_val);
	}

	if (isset($_POST['search_keyword_count'])) {
		$search = new Search;
		$search_valu = $_POST['search_key_valu'];
		$search->get_count_keyword($search_valu);
	}

	if (isset($_POST['search_keyword_clear'])) {
		$discover = new Users_Discover;
		$discover->get_wall_discover();
	}
